...
int check_for_local_domain(char *name, time_t now)
{
  ...
  for (spf = daemon->spf; spf; spf = spf->next)
    if (hostname_isequal(name, spf->name))
      return 1;
		      ...
		      struct in_addr local_addr, struct in_addr local_netmask, time_t now)
{
 ...
 rerun:
  ...
  if (qtype == T_SPF || qtype == T_ANY)
  {
    struct spf_record *t;
    for(t = daemon->spf; t ; t = t->next)
    {
      if (t->class == qclass && hostname_isequal(name, t->name))
      {
        ans = 1;
        if (!dryrun)
        {
          log_query(F_CNAME | F_FORWARD | F_CONFIG | F_NXDOMAIN, name, NULL, "<SPF>");
          if (add_resource_record(header, limit, &trunc, nameoffset, &ansp,
            daemon->local_ttl, NULL,
            T_SPF, t->class, "t", t->len, t->spf))
            anscount++;
